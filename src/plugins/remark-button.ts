		...
		visit(tree, (node: Node, index?: number, parent?: Parent) => {
			...
			if (directiveNode.name === 'btn' || directiveNode.name === 'button') {
				...
				if (directiveNode.children && Array.isArray(directiveNode.children)) {
					processedChildren = directiveNode.children.reduce(
						(acc: PhrasingContent[], child: Content) => {
							...
							if (child.type === 'paragraph') {
								...
								child.type === 'emphasis' ||
								child.type === 'strong' ||
								child.type === 'inlineCode' /* other phrasing types */
							) {
				...
				const newButtonNode = h(
					...
					{
						class:
							'bg-skin-card rounded p-2 font-medium hover:text-skin-active inline hover:text-skin-active', // Default classes
