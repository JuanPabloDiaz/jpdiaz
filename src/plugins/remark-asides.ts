...
interface IconPaths {
	note: MdastHtmlNode[];
	tip: MdastHtmlNode[];
	caution: MdastHtmlNode[];
	danger: MdastHtmlNode[];
}
			...
			remove(directiveNode, (child: Node) => {
				...
				if (child.data && 'directiveLabel' in child.data && child.data.directiveLabel) {
					...
					if (
						...
						(child as Parent).children.length > 0
					) {
