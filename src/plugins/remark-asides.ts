...
import type { Paragraph, Text, PhrasingContent } from 'mdast'; // mdast specific types
import { remove } from 'unist-util-remove';
import { visit } from 'unist-util-visit';
// Assuming mdast-util-directive exports this type. If not, we might need a custom one.
import type { ContainerDirective } from 'mdast-util-directive';
import { t } from '../i18n/utils'; // Removed .ts extension

const variants: ReadonlySet<AsideVariant> = new Set(['note', 'tip', 'caution', 'danger']);

type AsideVariant = 'note' | 'tip' | 'caution' | 'danger';

interface AsideOptions {
  label?: (variant: AsideVariant) => string;
}

// Type for the custom h/s function return value
interface MdastHtmlNode extends Paragraph {
  data: {
    hName: string;
    hProperties: HastProperties;
  };
  children: PhrasingContent[]; // Ensure children are valid PhrasingContent
}

function defaultLabel(v: AsideVariant): string {
  const defaultText = v.charAt(0).toUpperCase() + v.slice(1);
  switch (v) {
    case 'note':
      return t('aside.note') || defaultText;
    case 'tip':
      return t('aside.tip') || defaultText;
    case 'caution':
      return t('aside.caution') || defaultText;
    case 'danger':
      return t('aside.danger') || defaultText;
    default:
      // This case should not be reached if v is AsideVariant
      return '';
  }
}
...
function h(
  ...
  return {
    type: 'paragraph', // This is unconventional; typically 'html' or custom node type
...
function s(
  el: string,
  attrs: HastProperties = {},
  children: PhrasingContent[] = [] // Typically SVG children might be specific too
): MdastHtmlNode {
  const { tagName, properties } = _s(el, attrs);
    type: 'paragraph',
    data: { hName: tagName, hProperties: properties },
    children, // children are expected to be PhrasingContent[] by MdastHtmlNode
  };
}

// Type for iconPaths
type IconPathData = MdastHtmlNode[]; // Each icon is an array of MdastHtmlNode (from s() calls)
interface IconPaths {
  note: IconPathData;
  tip: IconPathData;
  caution: IconPathData;
  danger: IconPathData;
  [key: string]: IconPathData; // Allow other keys, though only specific ones are used
}
 ...
 */
const remarkAsides: Plugin<[AsideOptions?], Node, Node> = (options?: AsideOptions) => {
  const settings = { label: defaultLabel, ...options };
  const isAsideVariant = (s: string): s is AsideVariant => variants.has(s as AsideVariant);
  ...
  // Type for the transformer function
  const transformer = (tree: Node): void => {
    visit(tree, (node: Node, index?: number, parent?: Parent) => {
      // Ensure node is a ContainerDirective, and parent and index are valid
      if (!parent || typeof index !== 'number' || node.type !== 'containerDirective') {
        return;
      }
      const directiveNode = node as ContainerDirective; // Cast to ContainerDirective

      const variant = directiveNode.name;
      if (!isAsideVariant(variant)) return;

      let title = settings.label?.(variant) || ''; // Ensure title is always string

      // Type for child in remove callback
      remove(directiveNode, (child: Node) => {
        ...
        if (child.data && 'directiveLabel' in child.data && child.data.directiveLabel) {
          // Ensure child has children and the first child has a value
          if ('children' in child && Array.isArray((child as Parent).children) && (child as Parent).children.length > 0) {
            ...
            if (firstChild.type === 'text' && 'value' in firstChild) {
              title = (firstChild as Text).value;
            }
          }
      ...
      // Ensure children of directiveNode are PhrasingContent if that's what h() expects for its last arg.
      // The original 'node.children' (which is directiveNode.children) are likely 'Content[]' from mdast.
      // This might need careful handling if directiveNode.children are not directly compatible with PhrasingContent.
      // For now, we cast it, but this could be a source of type errors if the actual content is e.g. a list.
      const contentChildren = directiveNode.children as PhrasingContent[];

      const asideMdast: MdastHtmlNode = h(
        'aside',
        {
          'aria-label': title || variant, // Use title for aria-label if available
        ...
        [
          h('h4', { class: 'remark-aside__title', 'aria-hidden': 'true' }, [
            s(
              ...
              iconPaths[variant] // This is IconPathData (MdastHtmlNode[])
            ),
            { type: 'text', value: title } as Text, // Explicitly type the text node
          ]),
          h('div', { class: 'remark-aside__content' }, contentChildren),
        ]
      );

      parent.children[index] = asideMdast as Node; // Replace node in parent
    });
  };

  return transformer; // Returns the transformer function
};

export { remarkAsides };
// If default export is preferred:
// export default remarkAsides;
