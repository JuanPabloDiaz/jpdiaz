...
export async function getGitHubCommits(repoUrl: string, author: string): Promise<GitHubCommitData | null> {
  try {
    const repoDetails = getRepoDetails(repoUrl);
    if (!repoDetails) return null;
    const { owner, repo } = repoDetails;
    ...
    try {
      const statsResponse = await githubRequest<GitHubContributor[]>(`/repos/${owner}/${repo}/stats/contributors`);
